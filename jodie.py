...
def train():
    ...
    for batch in train_loader:
        ...
        # )
        # breakpoint()
        root_ts = torch.cat([t, t, t], dim = 0).double().to(device)
        root_nodes = torch.cat([src, pos_dst, neg_dst], dim = 0).to(device)
...
def test(loader, neg_sampler, split_mode):
    ...
    for pos_batch in loader:
        ...
        for idx, neg_batch in enumerate(neg_batch_list):
            ...
            root_ts = torch.full((2 + 2*len(neg_batch),), pos_t[idx], device=device)
            dst = torch.tensor(
                np.concatenate(
            ...
            root_nodes = torch.cat([src, dst], dim = 0).to(device)
            ...
            y_pred = model['link_pred'](z[:src.shape[0]], z[src.shape[0]:2*src.shape[0]])
...
for run_idx in range(NUM_RUNS):
    ...
    for epoch in range(1, NUM_EPOCH + 1):
        ...
        print(
            f"Epoch: {epoch:02d}, Loss: {loss:.4f}, Training elapsed Time (s): {tim:.4f}"
        )
