...
from utils import get_msg_func, get_emb_module, get_agg_module
...
# device = args.device
CN_TIME_DECAY = False
MAX_TR_TIME = 72*60*60
MAX_EXEC_TIME = 96*60*60
NUM_NEIGHBORS = 10
HOP_NUM = 2
NCN_MODE = 2
PER_VAL_EPOCH = 1
...
# data loading
# dataset = PyGLinkPropPredDataset(name=DATA, root="../../../../../../../../../work/pi_mserafini_umass_edu/ashraful/datasets")
...
msg_module = get_msg_func(args.msg_func, data, MEM_DIM, TIME_DIM)
...
gnn = get_emb_module(args.emb_func, data, MEM_DIM, EMB_DIM, memory, device)
...
results_filename = f'{results_path}/{MODEL_NAME}_{DATA}__{BATCH_SIZE}_{NCN_MODE}_{args.msg_func}_{args.emb_func}_{args.agg_func}_{args.nei_sampler}_{NUM_NEIGHBORS}_results.json'
...
for run_idx in range(NUM_RUNS):
    ...
    for epoch in range(1, NUM_EPOCH + 1):
        ...
        if epoch % PER_VAL_EPOCH == 0:
            ...
            perf_metric_val, trans_val, ind_val = test(val_loader, neg_sampler, split_mode="val", val_neg=args.val_neg)
    ...
    perf_metric_test, trans_test, ind_test = test(test_loader, neg_sampler, split_mode="test", val_neg = args.val_neg)
