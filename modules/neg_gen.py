...
class NegativeEdgeGenerator(object):
    ...
    def generate_negative_samples_hist_rnd(
        ...
        if os.path.exists(filename):
            ...
            # all possible destinations
            optimized_neg_sampling = True


            all_dst = np.arange(self.first_dst_id, self.last_dst_id + 1)

            if optimized_neg_sampling:
                np.random.shuffle(all_dst)
            ...
            for idx, (
                ...
                if not optimized_neg_sampling:
                    ...
                    if  len(all_dst) < num_rnd_neg_e + 1:
                        neg_rnd_dsts = np.setdiff1d(all_dst, invalid_dst)
                    else:
                        if idx % 500000 == 0:
                            np.random.shuffle(all_dst)
                        starting_point = np.random.randint(0, len(all_dst)-num_rnd_neg_e)
                        neg_rnd_dsts = all_dst[starting_point: starting_point+num_rnd_neg_e]
                        neg_rnd_dsts = neg_rnd_dsts[~np.isin(neg_rnd_dsts, invalid_dst)]

                    # to be removed
                    # neg_rnd_dsts = np.random.choice(
                    #      all_dst, num_rnd_neg_e, replace=False)
